...
객체 식별성(Object identity; 두 객체가 물리적으로 같은가)이 아니라 논리적 동치성을 확인해야 할때, equals를 재정의 해야한다.

equals 메서드를 재정의할 때는 반드시 일반 규약을 따라야 한다. 다음은 Object 메서드를 재정의할 때는 반드시 아래 일반 규약을 따라야한다.
