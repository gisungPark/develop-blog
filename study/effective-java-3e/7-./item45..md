...
스트림 파이프라인은 되풀이는 되는 계산을 주로 함수 객체(람다, 메서드 참조)를 이용한다. 이는 for문에서 사용되는 코드블럭과는 차이점을 가진다.
...
* 한 데이터가 파이프라인의 여러 단계를 통과할 때 이 데이터의 각 단계에서 값 들에 동시에 접근하기 어려운 경우
  * 스트림 파이프란인은 한 값을 다른 값에 매핑하고 나면 원래 값을 잃는 구조이기 때문이다.
